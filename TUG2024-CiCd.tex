...
\DeclareRobustCommand{\TeXLive}{\acro{\TeX\,Live}\xspace}
\DeclareRobustCommand{\DEPP}{\acro{DEPP}\xspace}
% @Karl: Why is the \ConTeXt definition not robust? by using it that way it's a bit hard to keep it as a logo within the bib file

\let\origConTeXt\ConTeXt
\DeclareRobustCommand{\ConTeXt}{\origConTeXt}
...
These tools focus on writing documents and not on the creation of templates or packages.
...
This article shows how to get started using automated workflows on platforms like GitHub, Forgejo, or GitLab for document authors as well as \TeX{} developers.
\end{abstract}

\section{Introduction}
Every kind of software went into or is moving towards the cloud.
...
However there also is a third variant to compile documents which can be used also to improve package development and in general the stability of \LaTeX:
...
Therefore have to be reasons why it might be worth reading this article or even integrating the mechanisms into projects.

Early adopters of \acro{CI/CD} techniques in the \TeX{} ecosystems tried to follow the current state of OpenSource development and open a door for contributors in the development process.
For example, the \LaTeX{} Project currently creates a lot of user interaction via their GitHub projects~\cite{latex3-github} and also takes contributions from which the whole \TeX/\LaTeX{} community will profit.
...
There are tons of reasons why a compilation may succeed on one system and fail on another.
Running some external \acro{CI/CD} pipeline will not only illustrate the necessary steps to go from source to the full \acro{PDF}.
...
Sometimes, a \LaTeX{} package may break when some of its dependencies are updated.
With \acro{CI/CD}, one can periodically test wether a package works in any number of TeX distributions and their versions.
This is especially useful to test if there are any issues with some package update before updating a machine, e.\,g., installing the latest MikTeX updates  where downgrading may be complicated.\footnote{That's another issue to address … but a different story.}

Additionally, template developers can also use it to check backwards compatibility, e.\,g., if one collaborator is required to use the version shipped with Debian Stable which is stuck on some outdated version.
...
\end{enumerate}
%
\noindent GitHub provides pre-configured actions which are able to combine multiple steps.
For example the latex-action \cite{latex-action} starts another container inside the action container to run \command{latexmk}.
However, those actions usually limit the configuration options to simplify the interface.
We are going to elaborate on two variants: Using a container image with a full installation of \TeXLive (this section) and a minimal installation (see Section~\ref{sec:minimize}).

Creating a YAML file hello-world.yml in the subdirectory .github/workflows/ of the repository".

The configuration for an action is done by creating a YAML file within the repositories' subdirectory \file{.github/workflows/}.
The filename may be chosen individually.
Here we use \file{.github/workflows/hello-world.yml}.
...
A full list of the readily available runners and detailed description of the images can be found within the GitHub documentation \cite{github-hosted-runners}.

\item[steps:] This is what the workflow should actually do.
As one can see it is possible to directly enter shell commands in there.
...
\meta{repository url}\url{/actions}. For the first of our demo repositories, this can be found at
...
If the pipeline succeeds, there will be a green checkmark, but we will fail to find the PDF anywhere.
...
Sadly, GitHub currently does not provide an option for individual files.
Furthermore, it is not possible to have a static link pointing to the latest artifacts.
...
GitLab CI is not that different from the previous Actions setups.
...
Thus, the file \file{README.md} of the demo repositories there include a link of the following structure:
...
It is also possible to use those in separate actions or even workflow files.
...
Thus, the first run will use \TeXLive~2022, continue on 2023 and finally run on the latest release.
...
\setListingIcon{}
Within the tutorial at TUG2024, there was a short journey to running the pipelines locally.
...
For the platforms which use actions there is a tool called \enquote{act} to simplify that process using the Docker API~\cite{act}.
%Forgejo is extending this tool and using it for their runners\cite{forgejo-runner}.

GitLab extends this by the option to simply have a GitLab runner installed locally.
...
The same goes for study material where we have rendered version including solutions and some without or documents which share lots of cross-references and therefore require a lot of runs.
...
We explained the use of GitHub, Forgejo, and GitLab for compiling \LaTeX{} documents and testing packages.
...
Also we  took care of being able to access the PDF or the testing results.
...
This can also be a preparation for next years \TeXLive{} pretest, as the Island of \TeX{} also is creating a Docker image for the pretests.
...
\meta{ci-type}\file{\_}\meta{task} adding \enquote{\file{\_minimal}} if the example is not using a pre-packaged Docker Image but includes methods to install packages based on a dependency file as described in Section~\ref{sec:minimize}. –
All variants listed here have been prepared for at least the 3 platforms mentioned in the article.
