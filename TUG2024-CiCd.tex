...
Both a private and a public repository work equally well.
...
As a consequence, we are going to elaborate on two variants: Using a full container image (this section) and a minimal installation (Sect.~\ref{sec:min}).
...
Be aware that for repository forks, actions need to be enabled explicitly in the tab \enquote{Actions}.
...
The setup of the images is described within~\cite{islandoftex-docker-short}.
...
\begin{tugboatlisting}[breaklines]{yaml}
- name: move pdf
...
Thus, the first run will be on \enquote{texlive:TL2022-historic}, the next on \enquote{texlive:TL2023-historic}, and the final one  on \enquote{texlive:latest}.
...
Still, sometimes one will not need all the bells and whistles and there are advantages to smaller build containers.
For instance, the full \TeXLive image requires more than one gigabyte and can take quite some time for downloading.
Moreover, one may only have limited disk space available on the runner server.

Side remark: If you are aiming to reduce the compile time (and not the image size itself), because you are building many documents, have a look at last year's IoT article~\cite{islandoftex-docker}.
...
As a consequence, the Island of \TeX{} proudly presents: \DEPP – The DEPendency Printer for \TeXLive\cite{depp}.\footnote{If you are German, do not be surprised if this tool is more clever than its name suggests.}
As this article should focus on pipelines we will not go into detail on the tool itself but instead show how \DEPP produces a file listing all \TeXLive packages necessary for the build.
...
On GitHub there are (at least) two actions installing \TeXLive as a part of the workflow~\cite{marcel-latex-action,teatimeguest-latex-action}.
...
Currently, these actions are continuously improved.
Currently, \action{zauguin/install-texlive} does a very good job at mirror selection, whereas \action{teatimeguest/setup-texlive-action} supports running on historic \TeXLive{} releases.
...
Another option is to provide a container image which already contains the necessary packages.
...
Again, GitLab directly supports always uploading test results without loading external extensions:
...
For example if one wants to use different artifacts for success and failure, it is required to do that within two separate jobs within GitLab.
