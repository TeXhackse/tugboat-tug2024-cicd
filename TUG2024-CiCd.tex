...
\setminted{xleftmargin=1em,numbersep=\mintednumbersep,numbers=left,breakaftersymbolpre={}}
...
\begin{abstract}
  ...
  That option is especially interesting when one versions the project
  using git.
  This tutorial will show you how to get a \LaTeX\ document compiled and shared using a build server hosted by GitHub, GitLab, or Forgejo.
  % At first,
  % concepts of git and GitHub\slash GitLab will be explained. This includes
  % the concept of branches, pushes, automated checks on the build server,
  % and collaborative work using pull\slash merge requests. After that,
  % there will be time for practical exercises and questions.
  %
  % Collaboration for editing \LaTeX{} documents using web-based platforms is common these days.
  % It is not well known that social coding platforms such as GitHub and GitLab can also be used for sharing \LaTeX{} code and compiled documents.
  % This tutorial walks through the use of GitHub and GitLab as platforms for compiling and sharing build results.
\end{abstract}

\section{Introduction}
Nearly everything went into or is moving towards the cloud.
\LaTeX{} is already there for about 10 years and today it is quite common to use a web editor for collaboration and local compilation became the \enquote{nerdy} way.
...
adapting DevOps methods, such as continuous integration and delivery (\acro{CI/CD}) using automated workflows.
...
\subsection{Why continuous integration?}
Having an established workflow usually makes people avoid thinking about changing anything.
However, there have to be reasons why it might be worth reading this article or even integrating the mechanisms into projects.
...
% TODO: use https://www.tug.org/TUGboat/tb40-3/tb126island-docker.pdf for the description
...
\noindent\includegraphics[width=\linewidth]{screenshot-artifacts}
...
This will force the step to run also if the step before failed.
